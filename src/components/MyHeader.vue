<script>
import { mapState, mapGetters } from 'vuex'

export default {
    computed: {
        ...mapState({
            costCart: state => state.cart.cost,
            countCart: state => state.cart.count
        }),
        ...mapGetters({
            getCountCart:'cart/getCountCart',
            getCostCart:'cart/getCostCart',
        })
    }
}
</script>
<template>
    <header class="header">
        <div class="header__block">
            <div class="header__logo" @click="$router.push('/')">
                <img src="src/assets/images/logo.svg" alt="" class="header__logo-img">
            </div>
            <div class="header__title">
                <p class="header__title-name">PIZZA STORE</p>
                <p class="header__title-text">самая вкусная пицца во вселенной</p>
            </div>
        </div>
        <div v-if="$route.meta.showCart" class="header__cart">
            <router-link 
                class="header__cart-btn"
                to="/cart"
            >
                <span class="header__cart-cost">{{ getCostCart }} ₽</span> 
                <span class="header__cart-hr"></span>
                <span class="header__cart-count">
                    <svg class="header__cart-icon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                        <path d="M6.33333 16.3333C7.06971 16.3333 7.66667 15.7364 7.66667 15C7.66667 14.2636 7.06971 13.6667 6.33333 13.6667C5.59695 13.6667 5 14.2636 5 15C5 15.7364 5.59695 16.3333 6.33333 16.3333Z" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M14.3333 16.3333C15.0697 16.3333 15.6667 15.7364 15.6667 15C15.6667 14.2636 15.0697 13.6667 14.3333 13.6667C13.597 13.6667 13 14.2636 13 15C13 15.7364 13.597 16.3333 14.3333 16.3333Z" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M4.78002 4.99999H16.3334L15.2134 10.5933C15.1524 10.9003 14.9854 11.176 14.7417 11.3722C14.4979 11.5684 14.1929 11.6727 13.88 11.6667H6.83335C6.50781 11.6694 6.1925 11.553 5.94689 11.3393C5.70128 11.1256 5.54233 10.8295 5.50002 10.5067L4.48669 2.82666C4.44466 2.50615 4.28764 2.21182 4.04482 1.99844C3.80201 1.78505 3.48994 1.66715 3.16669 1.66666H1.66669" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <span>{{ getCountCart }}</span>
                </span>
            </router-link>
        </div>
    </header>
</template>
<style scoped lang="scss">
.header {
    display: flex;
    justify-content: space-between;
    background: var(--white);
    margin: 0 12px;
    padding: 50px 26px 40px 65px;
    border-bottom: 1px solid #F6F6F6;

    &__block {
        display: flex;
        align-items: center;
    }

    &__logo {
        width: 38px;
        height: 38px;
        cursor: pointer;

        &-img {
            width: 100%;
            height: 100%;
            object-fit: cover;
        }
    }

    &__title {
        display: flex;
        flex-direction: column;
        margin-left: 17px;

        &-name {
            color: var(--text-black);
            font-size: 24px;
            font-weight: 800;
        }

        &-text {
            color: var(--text-gray-dark);
        }
    }

    &__cart {
        margin-left: auto;

        &-btn {
            background: var(--orange);
            padding: 12px 23px 12px;
            border-radius: 30px;
            border: none;
            display: flex;
            height: 50px;
            align-items: center;
            cursor: pointer;
            transition: all 0.2s ease;
            text-decoration: none;

            &:hover {
                opacity: 0.8;
            }
        }

        &-cost {
            color: var(--white);
            font-size: 16px;
            font-weight: 700;
            text-align: center;
        }

        &-hr {
            width: 1px;
            height: 100%;
            background: rgba(255, 255, 255, 0.25);
            margin: 0 13px;
        }

        &-icon {
            width: 16px;
            height: 16px;
            margin-right: 8px;
        }

        &-count {
            display: flex;
            align-items: center;
            color: var(--white);
            font-size: 16px;
            font-weight: 700;
        }

    }

}
</style>